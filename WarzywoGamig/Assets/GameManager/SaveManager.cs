...
public class SaveManager : MonoBehaviour
{
    ...
    // DODATKOWA METODA: Pobierz nazwÍ slotu z pliku (np. do wyúwietlania w menu)
    ...
    // Ustaw nazwÍ slotu przed zapisaniem!
                ...
                if (inventory != null)
                {
                    foreach (var weaponName in inventory.weapons)
                    {
                        if (!string.IsNullOrEmpty(weaponName))
                        {
                            ...
                            // Jeúli to broÒ aktualnie trzymana ñ pobierz z instancji
                            ...
                            // Dla pozosta≥ych ñ pobierz ze s≥ownika AmmoState
                            ...
                            // Jeúli nie ma jeszcze w s≥owniku (np. nowa broÒ) ñ domyúlne wartoúci
                            else
                            {
                    ...
                    Debug.Log($"[SaveManager] SavePlayerData: zaczynam zapisywaÊ usableItems, inventory.usableItems.Count = {inventory.usableItems.Count}");
                        ...
                        if (usableObj != null)
                        {
                            ...
                            if (interactable != null)
                            {
                                ...
                                if (treasure != null)
                                {
                                    foreach (var cat in treasure.resourceCategories)
                                    {
                                        Debug.Log($"[SaveManager] DodajÍ kategoriÍ do usableItem: {cat.name}, iloúÊ: {cat.resourceCount}");
    ...
    public void LoadPlayerData(int slotIndex)
    {
        ...
        try
        {
            ...
            // Przy wczytaniu zapisz nazwÍ slotu do currentSlotName (moøe byÊ przydatne)
    ...
    private IEnumerator LoadMainThenTargetScene(PlayerData data)
    {
        ...
        if (inventory != null)
        {
            // --- PRZYWR”∆ STAN AMUNICJI KAØDEJ BRONI ---
            ...
            Debug.Log($"[SaveManager] LoadPlayerData: czyszczÍ inventory.usableItems. Przed czyszczeniem: {inventory.usableItems.Count}");
            ...
            {
                Debug.Log($"[SaveManager] PrÛbujÍ odtworzyÊ usableItem: {usableSave.itemName}, typ: {usableSave.usableType}, kategorii: {usableSave.resourceCategoriesData?.Count ?? 0}");
                ...
                if (inventory.itemPrefabs.TryGetValue(usableSave.itemName, out var prefab) && prefab != null)
                {
                    ...
                    if (treasure != null && usableSave.resourceCategoriesData != null)
                    {
                        ...
                        foreach (var cat in usableSave.resourceCategoriesData)
                        {
                            ...
                            rc.resourceCount = cat.resourceCount;
                            treasure.resourceCategories.Add(rc);
                            Debug.Log($"[SaveManager] Dodano kategoriÍ: {rc.name}, iloúÊ: {rc.resourceCount}");
                        }
                    }
                    else
                    {
                    ...
                    if (interactable != null)
                    {
                        // OdtwÛrz typ usable (jeúli trzeba)
